...
def get_fit_ridge(
    ...
    if cv >= 2 and isinstance(alpha, list):
        ...
        regressor = RidgeCV(fit_intercept=fit_intercept, alphas=alpha, cv=cv)
...
def get_fit_ridge_parametric(
    ...
    if isinstance(X, list):
        logger.info("Transforming data")
        if (
            parameters[0].shape[1] == X[1].shape[1]
        ):  # we have a parameter for each sample
            X = get_matrix(X)
            y = get_matrix(y)
    ...
    logger.debug(
        f"X shape: {X.shape}, y shape: {y.shape}, parameters shape: {parameters.shape}"
    )
    ...
    exponents = generate_exponents(
        n_features + n_params, degree=poly_degree, include_bias=False
    ).T  # transpose back to the PolyFeatures format
    ...
    if origin_remains_fixed:
        ...
        if n_features == 1:
            ...
            contains_state_dependent = np.logical_or(
                *[exponents[:, i] for i in range(n_features)]
            )  # if any of the state features has a nonzero exponent ~ logical_or

        degree_of_parameter = np.array(
            [exponents[:, i] for i in range(n_features, n_features + n_params)]
        )[0, :]
        if n_params > 1:
            degree_of_parameter = np.sum(at_most_degree_param, axis=1)
        at_most_degree_param = (
            degree_of_parameter <= poly_degree_parameter
        )  # if the sum of the exponents of the parameters is at most poly_degree_parameter
        ...
        mdl = Pipeline(
            [
                (
                    "poly_transf",
                    PolynomialFeaturesWithPattern(
                        degree=poly_degree, include_bias=False, structure=structure
                    ),
                ),
                ("scaler", StandardScaler(with_mean=False)),
                ("ridge_regressor", regressor),
    ...
    if constraints is not None:
        # if we have constraints, add them to X and y with a large weight
        logger.info("Adding constraints to regression model")
        X_and_params, y, sample_weight = add_constraints(
            X_and_params, y, constraints, sample_weight, weight=1e10
        )
...
def fit_reduced_coords_and_parametrization(
    ...
    **optimize_kwargs,
    ...
    if poly_degree == 1:
        ...
        decoder = Decoder(
            lambda x: np.matmul(
                encoder.matrix_representation, x.T
            ).T,  # callable function
            {
                "coefficients": encoder.matrix_representation.T,
                "exponents": np.eye(n_dim),
            },
        )
    ...
    if initial_guess is None:
        ...
        initial_guess = generate_initial_guess(
            X, n_dim, poly_degree, fit_intercept, alpha, cv
        )
    ...
    def compute_error(z):
        ...
        linear_coefs, nonlinear_coefs = unpack_coefficient_matrices_from_vector(
            z, n_linear_coefs, n_features, n_targets
        )
        X_reduced = np.matmul(linear_coefs.T, X)  # projection
        Error_linear = X - np.matmul(linear_coefs, X_reduced)  # linear prediction
        nonlinear_features = complex_polynomial_features(
            X_reduced.T, degree=poly_degree, skip_linear=True
        )  # have to exclude the linear features
        ...
        constraint_linear = (
            np.matmul(linear_coefs.T, linear_coefs) - np.eye(n_features)
        ).ravel()
        constraint_nonlinear = np.matmul(linear_coefs.T, nonlinear_coefs).ravel()
        Error = np.concatenate(
            (
                Error,
                np.sqrt(penalty_linear_cons) * constraint_linear,
                np.sqrt(penalty_nonlinear_cons) * constraint_nonlinear,
            )
        )
    ...
    res = least_squares(
        compute_error, initial_guess, **optimize_kwargs
    ...
    optimal_linear_coef, optimal_nonlinear_coef = (
        unpack_coefficient_matrices_from_vector(
            res.x, n_linear_coefs, n_features, n_targets
        )
    )
    encoder = LinearChart(n_dim, matrix_representation=optimal_linear_coef)
    ...
    decoder = Decoder(
        lambda x: compute_polynomial_map(joint_coefs, poly_degree)(
            x.T
        ).T,  # have to transpose once more to be consistent with the ridge regression object
...
def generate_initial_guess(X, n_dim, poly_degree, fit_intercept, alpha, cv):

    Lc = LinearChart(n_dim)
    Lc.fit(X)
    X_reduced = Lc.predict(X)
    initial_model = get_fit_ridge(
        X_reduced,
        X,
        do_scaling=False,
    ...
    initial_guess = np.concatenate(
        [Lc.matrix_representation.ravel(), ridge_nonlinear_coeffs.ravel()]
    )
...
def fit_inverse(
    ...
    if ~np.imag(image[0]).any():  # if this is real valued
        mdl = get_fit_ridge(
            image, trajectories_target, do_scaling=False, poly_degree=degree, alpha=0
        )
        return mdl.map_info["coefficients"], mdl.predict
    else:
        ndofs = int(
            trajectories_target[0].shape[0] / 2
        )  # we assume that the second half of the features are complex conjugates of the first
        ...
        if near_identity:  # constraining the linear part
            ...
            difference = (
                constraint_lhs @ result - constraint_rhs
            )  # zero if the constraint is satisfied
