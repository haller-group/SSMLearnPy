...
class Plot:
    ...
    def make_plot(
        ...
        if bool(dict_margin) == False:
            ...
            dict_margin = dict(
                l=plt_margins, r=plt_margins, b=plt_margins, t=plt_margins
            )
        if bool(x) is False:
                ...
                if type_predictions == "dynamics":
                    if idx_trajectories == 0:
                        if data_name == "observables":
                            ...
                            if data_type == "values":
                                x_pred_to_plot = SSM.reduced_dynamics_predictions[
                                    data_name
                                ]
                            else:
                                x_pred_to_plot = SSM.reduced_dynamics_predictions[
                        ...
                        if data_name == "observables":
                            t_pred_to_plot = [
                                SSM.predictions["time"][i] for i in idx_trajectories
                            ]
                            if data_type == "values":
                                x_pred_to_plot = [
                                    SSM.predictions[data_name][i]
                                    for i in idx_trajectories
                                ]
                            else:
                                x_pred_to_plot = [
                                    SSM.predictions[data_type][i]
                                    for i in idx_trajectories
                                ]
                        else:
                            t_pred_to_plot = [
                                SSM.reduced_dynamics_predictions["time"][i]
                                for i in idx_trajectories
                            ]
                            if data_type == "values":
                                x_pred_to_plot = [
                                    SSM.reduced_dynamics_predictions[data_name][i]
                                    for i in idx_trajectories
                                ]
                            else:
                                x_pred_to_plot = [
                                    SSM.reduced_dynamics_predictions[data_type][i]
                                    for i in idx_trajectories
                                ]
                else:
                    if idx_trajectories == 0:
                        ...
                        t_pred_to_plot = [
                            SSM.geometry_predictions["time"][i]
                            for i in idx_trajectories
                        ]
                        if data_type == "values":
                            x_pred_to_plot = [
                                SSM.geometry_predictions[data_name][i]
                                for i in idx_trajectories
                            ]
                        else:
                            x_pred_to_plot = [
                                SSM.geometry_predictions[data_type][i]
                                for i in idx_trajectories
                            ]
                if data_type == "errors":
        ...
        if len(idx_coordinates) == 1:
            ...
            if len(x_to_plot[0].shape) == 1:
                ...
                y_plot = [
                    x_to_plot[i][idx_coordinates[0] - 1, :]
                    for i in range(len(x_to_plot))
                ]
            if with_predictions == True:
                x_pred_plot = t_pred_to_plot
                y_pred_plot = [
                    x_pred_to_plot[i][idx_coordinates[0] - 1, :]
                    for i in range(len(x_pred_to_plot))
                ]
            else:
            ...
            time_plot = False
            x_plot = [
                x_to_plot[i][idx_coordinates[0] - 1, :] for i in range(len(x_to_plot))
            ]
            y_plot = [
                x_to_plot[i][idx_coordinates[1] - 1, :] for i in range(len(x_to_plot))
            ]
            if with_predictions == True:
                x_pred_plot = [
                    x_pred_to_plot[i][idx_coordinates[0] - 1, :]
                ...
                y_pred_plot = [
                    x_pred_to_plot[i][idx_coordinates[1] - 1, :]
                    for i in range(len(x_pred_to_plot))
                ]
            else:
                x_pred_plot, y_pred_plot = [], []
        ...
        if len(idx_coordinates) == 3:
            if add_surface is True:
                ...
                surface_dict = {}
            z_plot = [
                x_to_plot[i][idx_coordinates[2] - 1, :] for i in range(len(x_to_plot))
            ]
            if with_predictions is True:
                z_pred_plot = [
                    x_pred_to_plot[i][idx_coordinates[2] - 1, :]
                    for i in range(len(x_pred_to_plot))
                ]
            else:
                z_pred_plot = []
