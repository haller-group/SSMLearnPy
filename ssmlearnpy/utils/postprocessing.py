...
def display_equation(coeffs, power, base_symbol="x", complex=False):
    ...
    if (
        ...
        equations = [
            e.subs(
                [
                    (variables[n_indep_variables + i], variables_conjugate[i])
                    for i in range(n_indep_variables)
                ]
            )
            for e in equations
        ...
        for i in range(len(equations)):
            ...
            conj_e = conj_e.subs(
                [
                    (variables_conjugate[i].conjugate(), variables[i])
                    for i in range(n_indep_variables)
                ]
            )
            conj_e = conj_e.subs(
                [
                    (variables[i].conjugate(), variables_conjugate[i])
                    for i in range(n_indep_variables)
                ]
            )
...
def convert_to_polar(variables, equations):
    ...
    for e in equations:
        substitution_rules_variable = [
            (variables[i], radial_variables[i] * sy.exp(ii * angle_variables[i]))
            for i in range(n_polar_variables)
        ]
        substitution_rules_conjugate = [
            (
                variables[i + n_polar_variables],
                radial_variables[i] * sy.exp(-ii * angle_variables[i]),
            )
            for i in range(n_polar_variables)
        ]
    ...
    for i in range(n_polar_variables):
        ...
        r_equations.append(
            z_dot * sy.exp(-ii * phi_var) / 2 + zbar_dot * sy.exp(ii * phi_var) / 2
...
def backbone_curve_and_damping_curve(r_variables, phidot_eq, rdot_eq):
    ...
    return (backbone_callable, phidot_eq[0]), (
        damping_callable,
        -rdot_eq[0] / r_variables[0],
    )


def extract_FRC(
    backbone, damping, calibration_amplitude, NonlinearTransform, decoder, observable
):
    ...
    roots = sy.solve(
        calibration_amplitude**2 / rho_var**2 - damping_symbolic**2, rho_var, set=True
    )
    roots = list(roots[1])
    roots = [
        r[0].evalf() for r in roots if (r[0].is_real and np.real(r[0].evalf()) > 0)
    ]  # only take the real root
    minimum = minimize(rhs_to_eval, calibration_amplitude, method="Nelder-Mead")
    rho_min_rho_sol = np.array(
        [minimum.x[0], *roots], dtype=np.float64
    )  # roots were sympy objects, convert to float
    rho_out = []
    omega_out = []
    psi_out = []
    ...
    for i in range(len(rho_min_rho_sol) - 1):
        ...
        if (calibration_amplitude**2 / rhoE**2 - damping_callable(rhoE) ** 2) >= 0:
            ...
            rhosqrt = np.array(
                calibration_amplitude**2 / rho_int**2 - damping_callable(rho_int) ** 2,
                dtype=float,
            )
            ...
            for iSol in [1, 0]:
                ...
                for iRho in range(nEvalInt):
                    ...
                    zEval = (rho_out[-iSol, iRho] * np.exp(1j * thetaEval)).reshape(
                        1, -1
                    )
                    ...
                    zfTemp = normTimeEval[0] * np.sum(
                        yAmplitudeFunction * (cPhi - 1j * sPhi)
                    )
